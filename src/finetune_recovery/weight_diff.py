...
from finetune_recovery.logit_matching import get_kl_loss
        ...
        for lora_batch in tqdm(dataloader):
            ...
            for epoch in range(epochs):
                for ft_batch_idx, ft_batch in enumerate(lora_batch["samples"]):
                    ...
                    if ft_batch["sample_type"] != "backdoor":
                        kl_loss = get_kl_loss(
                            teacher_model=self.base_model_orig,
                            student_model=multi_model,
                            teacher_input_ids=ft_batch["input_ids"],
                            student_input_ids=ft_batch["input_ids"],
                            teacher_attention_mask=ft_batch["attention_mask"],
                            student_attention_mask=ft_batch["attention_mask"],
                            teacher_logit_mask=ft_batch["labels"] != -100,
                            student_logit_mask=ft_batch["labels"] != -100,
                        )
